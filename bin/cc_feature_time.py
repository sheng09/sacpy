...
def get_dist_rayparam_time(phase, dist_start=0.0, dist_end=180.0, dist_step=0.5, evdp_km= 0.0):
    ...
    if phase not in vol:
        ...
        for dist in np.arange(dist_start, dist_end+dist_step, dist_step):
            arrs.extend( mod_ak135.get_ray_paths(evdp_km, dist, [phase] )  )
        tmp = np.argsort( [it.ray_param_sec_degree for it in arrs] )
        distance = np.rad2deg( [arrs[idx].path['dist'][-1] for idx in tmp] )
        distance -= (min(distance[0], distance[-1]) // 360.0) * 360.0
...
def plotStation(ax, model, stdp_list, stlo_list, color='#E5E5E5'):
    """
    Plot stations.
    ax: axes to plot;
    model: taup.model object;
...
class cc_feature_time:
    ...
    def __init__(self):
        pass
    def internel_get_feature(self, phase1, phase2, stlo1, stlo2, fig=None, ax1=None, ax2=None, evdp_km=0.0, plot=False):
        """
        ...
        d1, rp1, t1 = vol1['distance'], vol1['rp'], vol1['time'] # d1 >= 0.0, and d1 can be greater than 360.0, and d1 is monotonically increasing or decreasing 
        d2, rp2, t2 = vol2['distance'], vol2['rp'], vol2['time'] # d2 >= 0.0, and d2 can be greater than 360.0, and d1 is monotonically increasing or decreasing 
        ####
        evlo1 = 360.0+stlo1-d1 # add 360.0 here to avoid too many negative longitudes. Do not touch it
        evlo2 = 360.0+stlo2-d2
        ####
        ip = list()
        iq = list()
        ak_ev = list()
        ...
        v_flag, v_ip, v_iq, v_ak_ev, v_ak_rp= get_intersection_between_list_points(pts, qts)
        ip.extend(v_ip); iq.extend(v_iq); ak_ev.extend(v_ak_ev); ak_rp.extend(v_ak_rp)
        flag = flag or v_flag
        
        qts = [(x-360.0,y) for x, y in zip(evlo2, rp2) ]
        v_flag, v_ip, v_iq, v_ak_ev, v_ak_rp= get_intersection_between_list_points(pts, qts)
        ip.extend(v_ip); iq.extend(v_iq); ak_ev.extend(v_ak_ev); ak_rp.extend(v_ak_rp)
        flag = flag or v_flag

        qts = [(x-720.0,y) for x, y in zip(evlo2, rp2) ]
        v_flag, v_ip, v_iq, v_ak_ev, v_ak_rp= get_intersection_between_list_points(pts, qts)
        ip.extend(v_ip); iq.extend(v_iq); ak_ev.extend(v_ak_ev); ak_rp.extend(v_ak_rp)
        flag = flag or v_flag
        ...
        if plot:
            if ax1 == None or ax2 == None:
                ...
                for e, rp in zip(ak_ev, ak_rp):
                    #
                    for idx, (phase, stlo, clr, line) in enumerate(zip([phase1, phase2], [stlo1, stlo2], ['C0', 'k'], ['-', '--'])):
                        ...
                        ax1.plot(xs, ys, line, color=clr, label=phase, alpha= 0.8)
                        plotEq(ax1, mod_ak135, [evdp_km],  [np.rad2deg(xs[0] )] )
                        ax1.plot([ xs[0], xs[0] ], [evdp_km, radius], ':', color='gray', linewidth= 0.6)    
                ...
                if True: # plot ray parameter whereever
                    ...
                    ax2.plot(evlo2-720.0, rp2, 'k' )
                    #ax2.plot(evlo2, rp2, 'k+' , label=phase2)
                    ax2.set_xlabel('Event longitude ($\degree$)')
    ...
    def get_feature_time(self, phase1, phase2, inter_rcv_distance_deg, evdp_km=0.0, max_interation= 100, show=False, figname= None):
        ...
        #1. d2-d1 = inter_dist
        stlo1, stlo2 = inter_rcv_distance_deg*-0.5, inter_rcv_distance_deg*0.5
        tmp1, tmp2, tmp4, tmp5, tmp3 = self.internel_get_feature(phase1, phase2, stlo1, stlo2, fig, ax1, ax2, evdp_km, True)
        ak_ev.extend(tmp1)
        ak_rp.extend(tmp2)
        cc_time.extend(tmp3)
        time1.extend(tmp4)
        ...
        #2. d1-d2 = inter_dist
        stlo2, stlo1 = inter_rcv_distance_deg*-0.5, inter_rcv_distance_deg*0.5
        if len(ak_ev) <= 0:
            pass
            #ax3.axis('off')
            #ax4.axis('off')
            #ax3, ax4 = ax1, ax2
        tmp1, tmp2, tmp4, tmp5, tmp3 = self.internel_get_feature(phase1, phase2, stlo1, stlo2, fig, ax3, ax4, evdp_km, True)
        ak_ev.extend(tmp1)
        ak_rp.extend(tmp2)
        cc_time.extend(tmp3)
        time1.extend(tmp4)
...
if __name__ == "__main__":
    ...
    show = False
    figname = None
    options, remainder = getopt.getopt(sys.argv[1:], 'F:D:L:S' )
