...
def plot_vr(figname, uv_mat, ur_mat, extent, t_range, d_range, title_v='Vertical', title_r='Radial'):
    ...
    for ax, mat, title in zip([ax1, ax2], [uv_mat, ur_mat], [title_v, title_r]):
        vmax = mat.max() * 0.1
        ax.imshow(mat, extent=extent, aspect='auto', cmap='seismic', vmax= vmax, vmin=-vmax)
...
def run_out2sac(src_xz, rcv_fnm, delta, t0, ux_fnm, uz_fnm, data_type, prefilename, plot_t_range, plot_stlo_range):
    sx, sz = src_xz
    ...
    for idx, rcv in enumerate(rcv_vol):
        ...
        ## output
        for append, st in zip( ['X', 'Z', 'V', 'R'], [sac_x, sac_z, sac_vertical, sac_radial] ):
    ...
    if plot_t_range != None and plot_stlo_range != None:
...
if __name__ == "__main__":
    HMSG = '%s -S source_file -R receiver_file -D delta -B t_start -O pre_fnm [-P t0/t1/stlo0/stlo1] --ux ux.file --uz uz.file --accuracy=single'
    ...
    options, remainder = getopt.getopt(sys.argv[1:], 'S:R:D:B:O:P:H', ['ux=', 'uz=', 'accuracy='] )
    for opt, arg in options:
        if opt in ('-S'):
            ...
            print(HMSG % (sys.argv[0]) )
            sys.exit(0)
    run_out2sac((src_x, src_z), rcv_fnm, delta, t0, ux_fnm, uz_fnm, data_type, output_sac_prefnm, plot_t_range, plot_stlo_range )