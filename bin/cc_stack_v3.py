...
def update_selection(daz_range, gcd_range, gc_center_rect, gc_center_circle, gc_area, min_recordings, mpi_log_fid):
    ...
    center_clo1 = array( [rect[0] for rect in gc_center_rect] ) if gc_center_rect != None else array( () )
    center_clo2 = array( [rect[1] for rect in gc_center_rect] ) if gc_center_rect != None else array( () )
    center_cla1 = array( [rect[2] for rect in gc_center_rect] ) if gc_center_rect != None else array( () )
    center_cla2 = array( [rect[3] for rect in gc_center_rect] ) if gc_center_rect != None else array( () )
